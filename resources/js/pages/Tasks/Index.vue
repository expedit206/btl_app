        ...
        <div class="container mx-auto px-6 py-8">
            ...
            <div class="grid gap-4 mb-10">
                <div v-for="task in pendingTasks" :key="task.id"
                    class="bg-theme-white border border-theme-black rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow duration-200 flex items-start">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-theme-black text-sm font-medium">Date : {{ task.date }} <span v-if="task.time">à
                                {{ task.time }}</span></p>
                    </div>
                </div>
            ...
            <inertia-link :href="route('tasks.create')" class="btn-primary mt-6 inline-block">
                Ajouter une tâche
            </inertia-link>
        </div>
    </AppLayout>
...
const pendingTasks = computed(() => tasks.filter(task => !task.status));
const completedTasks = computed(() => tasks.filter(task => task.status));
